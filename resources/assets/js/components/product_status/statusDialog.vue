		...
		<v-card>
			...
			<v-card-text>
				<v-form>
					<v-text-field 
					label="Status" 
					v-model="editedItem.product_status_name"
					:counter="25"
					...
					<v-text-field 
					label="Description" 
					v-model="editedItem.product_status_description"
					:counter="255"
...
export default {
	...
	methods: {
		...
		close: function() {
			...
			this.$store.commit('DIALOG_PRODUCT_STATUS_CLOSE')
			setTimeout(() => {
				this.errors.clear()		
			},300)
		},
		...
		save: function(request) {
			...
			if (vm.editedIndex > -1) {
				...
				vm.$validator.validateAll().then(function(result){
					if(result) {			
						vm.$store.dispatch('updateProductStatus', vm.editedItem).then(response => {
				...
				vm.$validator.validateAll().then((result) => {
					if(result) {						
						vm.$store.dispatch('addProductStatus', vm.editedItem).then(response => {
	...
	computed: {
		...mapState({
			dialog: state => state.productStatusStore.dialog,
			editedIndex: state => state.productStatusStore.editedIndex,
			item: state => state.productStatusStore.editedItem,
	...
	watch: {
		...
		'editedItem.product_status_name': function(val, oldVal) {
		...
		'editedItem.product_status_description': function(val, oldVal) {
			if(this.editedIndex > -1 && oldVal != '') {
				this.disabled = false
			} else if(this.editedIndex == -1 && val != '') {
				this.disabled = false
