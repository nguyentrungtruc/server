		...
		<v-card>
			...
			<v-card-text>
				<v-form>
					<v-text-field 
					label="Status" 
					v-model="editedItem.store_status_name"
					:counter="25"
					...
					<v-text-field 
					label="Description" 
					v-model="editedItem.store_status_description"
					:counter="255"
...
export default {
		...
		save: function(request) {
			...
			if (vm.editedIndex > -1) {
				...
				vm.$validator.validateAll().then(function(result){
					if(result) {			
						vm.$store.dispatch('updateStoreStatus', vm.editedItem).then(response => {
				...
				vm.$validator.validateAll().then((result) => {
					if(result) {						
						vm.$store.dispatch('addStoreStatus', vm.editedItem).then(response => {
	...
	computed: {
		...mapState({
			dialog: state => state.storeStatusStore.dialog,
			editedIndex: state => state.storeStatusStore.editedIndex,
			item: state => state.storeStatusStore.editedItem,
	...
	watch: {
		...
		'editedItem.store_status_name': function(val, oldVal) {
		...
		'editedItem.store_status_description': function(val, oldVal) {
			if(this.editedIndex > -1 && oldVal != '') {
				this.disabled = false
			} else if(this.editedIndex == -1 && val != '') {
				this.disabled = false
