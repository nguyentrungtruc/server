...
export default {
	...
	methods: {
		...
		close: async function() {
			...
			this.$store.commit('CLOSE_RATING_TYPE_DIALOG')
		},
		//Accept Update
		...
		//ACTION ADD NEW
		add(data) {
			const url = `RatingType/Add`
			RatingTypeRepository.create(data).then(response => {
				if(response.status === 201) {
					this.$store.dispatch('updateRatingType', response.data.type)
					this.close()
		...
		update(data) {
			const {id} = data
			RatingTypeRepository.update(id, data).then(response => {
	...
	computed: {
		...mapState({
			dialog     : state => state.ratingType.dialog,
			editedIndex: state => state.ratingType.editedIndex,
			item       : state => state.ratingType.editedItem,
