...
export default {
    ...
    methods: {        
        //ACTION BUTTON ADD
        addItem() {
            this.$store.commit('OPEN_RATING_TYPE_DIALOG')
        },
        //ACTION BUTTON EDIT
        editItem(item) {
        ...
        removeItem(item) {
            this.$refs.confirm.open('Remove Item', 'delete '+item.name+' type').then(result => {
                if(result) {
                    RatingTypeRepository.delete(item.id).then(response => {
                        if(response.status === 204) {
                            this.$store.dispatch('removeRatingType', item)
    ...
    computed: {
        ...mapState({
            loading: state => state.ratingType.loading,
            types  : state => [...state.ratingType.types],
        }),
        filterData() {
