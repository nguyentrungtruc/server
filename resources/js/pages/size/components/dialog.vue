...
export default {
	...
	methods: {
		...
		close: async function() {
			...
			this.$store.commit('CLOSE_SIZE_DIALOG')
		},
		//Accept Update
		...
		add(data) {
			...
			this.axios.post(url, data, {withCredentials: true}).then(response => {
				if(response.status === 201) {
					...
					this.close()
					this.success(data.name+' size has been added.')
				}
			}).catch(error => {
				if(error.response.status === 422) {
					this.fail(data.name+' size has already been taken.')
				}
		...
		update(data) {
			...
			this.axios.post(url, data, {withCredentials: true}).then(response => {
				if(response.status === 200) {
					this.$store.dispatch('updateSize', response.data.size)
					this.close()
					this.success(data.name+' size has been edited.')
				}
			}).catch(error => {
				if(error.response.status === 422) {
					this.fail(data.name+' size has already been taken.')
	...
	computed: {
		...mapState({
			dialog     : state => state.size.dialog,
			editedIndex: state => state.size.editedIndex,
			item       : state => state.size.editedItem,
			alert      : state => state.alert
		}),
		formTitle () {
			return this.editedIndex === -1 ? 'New Size': 'Edit Size'
		}
	},
	watch: {
		item (val) {
