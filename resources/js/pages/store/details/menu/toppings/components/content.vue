...
export default {
    ...
    methods: {        
        //ACTION BUTTON ADD
        addItem() {
            this.$store.commit('OPEN_TOPPING_DIALOG')
        },
        //ACTION BUTTON EDIT
        editItem(item) {
            this.$store.dispatch('editTopping', item)
        },
        //ACTION BUTTON REMOVE
        removeItem(item) {
            this.$refs.confirm.open('Remove Item', 'delete '+item.name+' topping').then(result => {
                if(result) {
                    const data = []
    ...
    computed: {
        ...mapState({
            loading : state => state.topping.loading,
            toppings: state => [...state.topping.toppings],
        }),
        filterData() {
            if(this.toppings.length > 0) {
                return this.filterByText(this.toppings, this.search.keywords)
            }
        }
    },
}
