...
export default {
    ...
    methods: {        
        //ACTION BUTTON ADD
        addItem() {
            this.$store.commit('OPEN_COUPON_STATUS_DIALOG')
        },
        //ACTION BUTTON EDIT
        editItem(item) {
        ...
        removeItem(item) {
            this.$refs.confirm.open('Remove Item', 'delete '+item.name+' coupon status').then(result => {
                if(result) {
                    const data = []
    ...
    computed: {
        ...mapState({
            loading: state => state.couponStatus.loading,
            status : state => [...state.couponStatus.status],
        }),
        filterData() {
