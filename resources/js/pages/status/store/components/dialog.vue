...
export default {
	...
	methods: {
		...
		close: async function() {
			...
			this.$store.commit('CLOSE_STORE_STATUS_DIALOG')
		},
		//Accept Update
		...
		//ACTION ADD NEW
		add(data) {
			StoreStatusRepository.create(data).then(response => {
				if(response.status === 201) {
					this.$store.dispatch('updateStoreStatus', response.data.status)
					this.close()
		...
		update(data) {
			const {id} = data
			StoreStatusRepository.update(id, data).then(response => {
	...
	computed: {
		...mapState({
			dialog     : state => state.storeStatus.dialog,
			editedIndex: state => state.storeStatus.editedIndex,
			item       : state => state.storeStatus.editedItem,
