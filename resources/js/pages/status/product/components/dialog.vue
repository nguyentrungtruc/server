...
export default {
	...
	methods: {
		...
		close: async function() {
			...
			this.$store.commit('CLOSE_PRODUCT_STATUS_DIALOG')
		},
		//Accept Update
		...
		//ACTION ADD NEW
		add(data) {
			ProductStatusRepository.create(data).then(response => {
				if(response.status === 201) {
					this.$store.dispatch('updateProductStatus', response.data.status)
					this.close()
		...
		update(data) {
			const {id} = data
			ProductStatusRepository.update(id, data).then(response => {
	...
	computed: {
		...mapState({
			dialog     : state => state.productStatus.dialog,
			editedIndex: state => state.productStatus.editedIndex,
			item       : state => state.productStatus.editedItem,
