...
export default {
    ...
    methods: {        
        //ACTION BUTTON ADD
        addItem() {
            this.$store.commit('OPEN_ACTIVITY_DIALOG')
        },
        //ACTION BUTTON EDIT
        editItem(item) {
        ...
        removeItem(item) {
            this.$refs.confirm.open('Remove Item', 'delete '+item.name+' activity').then(result => {
                if(result) {
                    const data = []
                    const url  = `/Activity/${item.id}/Remove`
                    this.axios.post(url, data, {withCredentials: true}).then(response => {
                        if(response.status === 204) {
                            this.$store.dispatch('removeActivity', item)
			                this.$store.dispatch('onAlert', {close: true, index: 0, message: item.daysofweek+' activity has been deleted.', routeName: this.$route.name, show: true, type: 'success'})
                        }
                    })
                }
            })
        ...
        filterByText(list, value) {
            ...
            return list.filter((item) => {
                return item.daysofweek.toLowerCase().includes(keywords)
            })
        }
    },
    computed: {
        ...
        filterData() {
            if(this.activities.length > 0) {
                return this.filterByText(this.activities, this.search.keywords)
            }
        }
    },
}
